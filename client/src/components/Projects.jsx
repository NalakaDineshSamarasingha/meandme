...
export default function Projects() {
  return (
  <section id="projects" className="w-full mx-auto max-w-[1500px] mt-[8vh]">
      <ul className="columns-1 md:columns-3 gap-8 [column-fill:_balance] w-[98%] mx-auto">
        {projects.map((p, idx) => (
          <motion.li
            key={p.slug}
          ...
          >
            <div
              className={`relative w-full overflow-hidden border border-zinc-800 bg-zinc-900 rounded-sm  ${p.aspect || ''}`}
            ...
            <div className="mt-5">
              {p.client && (
                <h3 className="text-[11px] font-semibold tracking-[0.18em] text-zinc-100 uppercase">{p.client}</h3>
              )}
              {p.category && (
                <p className="mt-2 text-[11px] tracking-[0.18em] text-zinc-500 uppercase">{p.category}</p>
              )}
